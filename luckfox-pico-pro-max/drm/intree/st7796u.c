...
/*
 * DRM driver for display panels connected to a Sitronix ST7796U
 * display controller in SPI mode.
 *
 * Copyright 2024 Iota Hydrae <writeforever@foxmail.com>
 */
...
static void yt350s006_init(struct mipi_dbi *dbi)
{
...
static void hp35006_init(struct mipi_dbi *dbi)
{
	mipi_dbi_command(dbi, 0xf0, 0xc3);
    mipi_dbi_command(dbi, 0xf0, 0x96);
				...
				struct drm_plane_state *plane_state)
{
	...
	if (of_device_is_compatible(dbidev->drm.dev->of_node, "yaoyuanhong,yt350s006")) {
		yt350s006_init(dbi);
	} else if (of_device_is_compatible(dbidev->drm.dev->of_node, "yaoyuanhong,hp35006")) {
		hp35006_init(dbi);
	} else {
		/*  fallback to default panel init sequence */
		yt350s006_init(dbi);
	}
...
static const struct of_device_id st7796u_of_match[] = {
	...
	{ .compatible = "yaoyuanhong,hp35006", .data = &yt350s006_cfg },
	{ },
};
