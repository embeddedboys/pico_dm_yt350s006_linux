...
/* note: taken from 6.1.99 drm_modes.c */
extern int of_get_drm_panel_display_mode(struct device_node *np,
				  ...
				  struct drm_plane_state *plane_state)
{
	...
	if (ret < 0)
		goto out_exit;
	if (!ret)
		panel_mipi_dbi_commands_execute(dbi, driver_private);
...
static struct drm_driver panel_mipi_dbi_driver = {
...
static int panel_mipi_dbi_spi_probe(struct spi_device *spi)
{
	...
	driver_private = panel_mipi_dbi_commands_from_fw(dev);
	if (IS_ERR(driver_private))
		return PTR_ERR(driver_private);
